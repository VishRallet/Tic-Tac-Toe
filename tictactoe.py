...
class Tic_Tac_Toe():
    def __init__(self):
        self.window = Tk()
        self.window.title('Vishal\'s Tic Tac Toe Game')
        self.canvas = Canvas(self.window, width=size_of_board, height=size_of_board, bg='#FCF3CF')
        self.canvas.pack()
    ...
    def initialize_board(self):
        for i in range(2):
            self.canvas.create_line((i + 1) * size_of_board / 3, 0, (i + 1) * size_of_board / 3, size_of_board, fill='#964B00', width=5)
        for i in range(2):
            self.canvas.create_line(0, (i + 1) * size_of_board / 3, size_of_board, (i + 1) * size_of_board / 3, fill='#964B00', width=5)
    ...
    def display_gameover(self):
        if self.X_wins:
            self.X_score += 1
            text = 'Bravo! Player-X Won'
            color = symbol_X_color
        elif self.O_wins:
            self.O_score += 1
            text = 'Bravo! Player-O Won'
            color = symbol_O_color
        else:
            self.tie_score += 1
            text = 'Oops! It was a Tie.'
            color = 'purple'
        ...
        score_text = 'Click anywhere to play again \n'
        self.canvas.create_text(size_of_board / 2, 15 * size_of_board / 16, font="cmr 20 bold", fill="purple", text=score_text)
